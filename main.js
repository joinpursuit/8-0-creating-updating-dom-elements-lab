let section = document.querySelector("section");// declaring a variable (point of reference) for the first section (child) element of main / there are 2 section elements (children) total 
section.classList.add("featured");// adding a class of featured to first section element

let posts = document.querySelector("section.posts");// setting a variable (point of reference) for the 2nd section / child element of main
let article = document.createElement("article");// creating an article element
posts.append(article);//appending new article element to the end of the 2nd section element, grabbed by class of .posts

let image = document.createElement("img");// seeting a variable for image element
image.setAttribute("src", "./images/paul-gilmore-unsplash.jpg");// setting attributes to img tag
image.setAttribute("alt","Image of a mountain in front of a lake.");
article.append(image);// appending image to article element.  img is now child of third article in .posts class
...
article.append(aside);// aside set as child of article, sibling of h3, p, & img
...
span.prepend(strong);// setting strong element, child of span, with text before span text content

let link = document.createElement("a");//anchor tag created
link.textContent = "Read more";// setting text for anchor
link.setAttribute("href", "#");// setting attribute for anchor tag
...
console.log(articles[1]);// grabbing the second article, with intention of swapping with first article[0] element 
posts.prepend(articles[1]);// swapping the place of the second article(child of section with class of .posts) with the first article element
